...
def seq_last_step(obs_seq: dict):
    """Берёт последний кадр из последовательности: 
       rgb/depth: (B, T, H, W, C) -> (B, H, W, C)
       state:     (B, T, S)       -> (B, S)
    """
    return {k: v[:, -1] for k, v in obs_seq.items()}
...
class Trans_PlainConv(nn.Module):
    def __init__(
        ...
        self.cnn = nn.Sequential(
            ...
            nn.MaxPool2d(4, 4) if image_size[0] == 128 and image_size[1] == 128 else nn.MaxPool2d(2, 2),
...
class Trans_EncoderObsWrapper(nn.Module):
    """Обёртка, которая умеет и train, и eval формы для ПОСЛЕДОВАТЕЛЬНОСТЕЙ изображения"""
    ...
    def forward(self, obs):
        ...
        if len(img.shape) == 5:
            ...
            img = img.reshape(n_e * cont, h, w, c).permute(0, 3, 1, 2)
            img = self.encoder(img)
            img = img.reshape(n_e, cont, self.encoder.out_dim)
        else:
            # train: (B, N_env, T, H, W, C)
            bs, n_e, cont, h, w, c = img.shape
            img = img.reshape(bs * n_e * cont, h, w, c).permute(0, 3, 1, 2)
            img = self.encoder(img)
            img = img.reshape(bs, n_e, cont, self.encoder.out_dim)
        return img

class PlainConv(nn.Module):
    def __init__(self, in_channels=3, out_dim=256, pool_feature_map=False, last_act=True, image_size=[128, 128]):
        super().__init__()
        self.out_dim = out_dim
        self.cnn = nn.Sequential(
            nn.Conv2d(in_channels, 16, 3, padding=1, bias=True), nn.ReLU(inplace=True),
...
class EncoderObsWrapper(nn.Module):
    """Простая обёртка для ОДНОГО состояния (без оси T) под критик."""
    def __init__(self, encoder):
        super().__init__()
        self.encoder = encoder
    ...
    def forward(self, obs):
        if "rgb" in obs:
            rgb = obs['rgb'].float() / 255.0   # (B, H, W, C)
        if "depth" in obs:
            depth = obs['depth'].float()
        if "rgb" and "depth" in obs:
            img = torch.cat([rgb, depth], dim=3)
...
class Actor(nn.Module):
    """Trans_Actor: CNN (Trans_PlainConv) + Transformer по временной оси"""
    def __init__(self, env, args, sample_obs):
        ...
        # посчитаем каналы и размер картинки по первому obs
        in_channels = 0
        image_size = None
        ...
        self.encoder = Trans_EncoderObsWrapper(
            Trans_PlainConv(in_channels=in_channels, out_dim=231, image_size=image_size) #228
        )
        ...
        self.action_scale = torch.as_tensor((env.single_action_space.high - env.single_action_space.low) / 2.0, dtype=torch.float32)
        self.action_bias  = torch.as_tensor((env.single_action_space.high + env.single_action_space.low) / 2.0, dtype=torch.float32)

    def get_feature(self, obs, detach_encoder=False):
        visual_feature = self.encoder(obs)  # (..., T, F)
        if detach_encoder:
            visual_feature = visual_feature.detach()
        x = torch.cat([visual_feature, obs['state']], dim=-1)  # (..., T, F+S)
        x = self.transformer(x)[:, -1, :]  # возьмём последний шаг
    ...
    def get_action(self, obs, detach_encoder=False):
        ...
        x_t = normal.rsample()
        y_t = torch.tanh(x_t)
        self.action_scale = self.action_scale.to(y_t.device)
        self.action_bias = self.action_bias.to(y_t.device)
...
class SoftQNetwork(nn.Module):
    """Критик SAC без трансформера, работает на ОДНОМ шаге"""
    def __init__(self, env, encoder: EncoderObsWrapper):
        super().__init__()
        self.encoder = encoder
        action_dim = int(np.prod(env.single_action_space.shape))
        self.state_dim = env.single_observation_space['state'].shape[0]
        self.mlp = make_mlp(self.encoder.encoder.out_dim + action_dim + self.state_dim, [512, 256, 1], last_act=False)

    def forward(self, obs, action, visual_feature=None, detach_encoder=False):
        if visual_feature is None:
            visual_feature = self.encoder(obs)       # (B, F)
        if detach_encoder:
            visual_feature = visual_feature.detach()
...
if __name__ == "__main__":
    ...
    if args.exp_name is None:
        ...
        run_name = f"ECAI_CAMERA_READY_RGB_ST2/{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}"
    else:
    ...
    _img_size = None
    if "rgb" in obs: 
        _in_ch += obs["rgb"].shape[-1]; _img_size = obs["rgb"].shape[1:3]
    if "depth" in obs:
        _in_ch += obs["depth"].shape[-1]; _img_size = obs["depth"].shape[1:3]
    critic_encoder = EncoderObsWrapper(PlainConv(in_channels=_in_ch, out_dim=256, image_size=_img_size))

    qf1 = SoftQNetwork(envs, critic_encoder).to(device)
    qf2 = SoftQNetwork(envs, critic_encoder).to(device)
    qf1_target = SoftQNetwork(envs, critic_encoder).to(device)
    qf2_target = SoftQNetwork(envs, critic_encoder).to(device)
    
    if args.checkpoint is not None:
        print('Downloading checkpoint!')
        ckpt = torch.load(args.checkpoint)
        actor.load_state_dict(ckpt['trans_actor'])
...
#↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        ...
        for local_update in range(args.grad_steps_per_iteration):
            ...
            with torch.no_grad():
                ...
                next_obs_1step = seq_last_step(data.next_obs)
                qf1_next_target = qf1_target(next_obs_1step, next_actions)
                qf2_next_target = qf2_target(next_obs_1step, next_actions)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_log_pi
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target).view(-1)

            obs_1step = seq_last_step(data.obs)
            qf1_a_values = qf1(obs_1step, data.actions).view(-1)
            qf2_a_values = qf2(obs_1step, data.actions).view(-1)
            ...
            if global_update % args.policy_frequency == 0:
                ...
                obs_1step = seq_last_step(data.obs)
                qf1_pi = qf1(obs_1step, pi, detach_encoder=True)
                qf2_pi = qf2(obs_1step, pi, detach_encoder=True)
